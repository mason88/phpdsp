...
use NumPHP\Core\NumArray\Outer;
...
class NumArray extends Cache
{
	...
	/**
     * Performs reciprocal on NumArray elements
     *
     * @return $this
     *
     * @throws DivideByZeroException will be thrown, when dividing by zero
	...
	/**
     * Performs outer product of an array or NumArray to the existing NumArray
     *
     * @param mixed $factor an other array or NumArray
     *
     * @return $this
     *
     * @api
     * @since 1.0.0
     */
    public function outer($factor)
    {
		if ($factor instanceof NumArray) {
            $factor = $factor->data;
        }
		
        $this->data = Outer::outer($this->data, $factor);
		$this->shape = Shape::getShape($this->data);
        $this->flushCache();
	...
	{
		$high = abs(max($this->getData()));
		$low = abs(min($this->getData()));
		$this->mult($max)->div(max($high, $low));
		
		return $this;
	}
	
	/**
     * Apply a function to value of the NumArray
     *
     * @return NumArray
	 *
	 * @api
